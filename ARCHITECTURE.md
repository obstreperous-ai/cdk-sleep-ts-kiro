...
4. **Synthesize Speech** - Invokes Amazon Polly `StartSpeechSynthesisTask` with configurable parameters (text from event input, voice: Joanna, format: MP3). The synthesized audio is written to the S3 Output Bucket. On failure, catches the error and transitions to the Mark Failed state.
   ...
   > **Note on dual Polly invocations:** The state machine's `Synthesize Speech` task (which calls `StartSpeechSynthesisTask`) is a separate Polly invocation from the Lambda's inline `SynthesizeSpeech`. The state machine task produces an independent audio output using a configurable placeholder text, while the Lambda processes the actual uploaded content (synthesizing speech from `.txt` files or passing through audio files). Both invocations are part of the pipeline's processing chain for audio enrichment -- the Lambda handles content-specific processing, and the state machine task provides an additional synthesized audio artifact downstream.
